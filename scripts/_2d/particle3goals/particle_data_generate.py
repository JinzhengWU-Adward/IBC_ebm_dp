...
# 导入 IBC 模块（3-goals 版本环境与 oracle）
from ibc.environments.particle import particle3goals
from ibc.environments.particle import particle3goals_oracles
...
def generate_particle_episode(env, oracle, max_steps=50):
    ...
    while not time_step.is_last() and step_count < max_steps:
        ...
        if step_count == 0:
            ...
            # 第三个目标：从 3-goals 环境的观测中直接读取独立的第三个目标
            third_goal = obs['pos_third_goal'].copy()
...
def generate_particle_dataset(
    ...
    # 创建 3-goals 环境（基于 IBC ParticleEnv 扩展）
    gym_env = particle3goals.ParticleEnv3Goals(
        n_steps=n_steps,
        n_dim=n_dim,
        hide_velocity=False,
        seed=seed,
    ...
    # 创建 3-goals Oracle 策略（需要包装后的环境）
    oracle = particle3goals_oracles.ParticleOracle3Goals(
        env,
        wait_at_goal=1,
        goal_threshold=0.01
    )
...
if __name__ == '__main__':
    # 输出路径
    output_dir = Path(__file__).parent.parent.parent.parent / 'data' / '_2d' / 'particle3goals'
