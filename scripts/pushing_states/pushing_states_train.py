"""
使用 PyTorch 复刻 IBC 的 Block Pushing (states) 环境训练
基于 run_mlp_ebm_langevin.sh 和 mlp_ebm_langevin.gin 的配置
"""
import os
import json
import numpy as np
...
# 添加项目路径
# pushing_states_train.py 位于: IBC_ebm_dp/scripts/pushing_states/pushing_states_train.py
# 需要找到 IBC_ebm_dp 目录
# pushing_states_train.py 位于: IBC_ebm_dp/scripts/pushing_states/pushing_states_train.py
# 需要向上3级到达 IBC_ebm_dp 根目录
...
def flatten_observation(obs_dict: dict) -> np.ndarray:
    """
    将 BlockPush 环境的观测字典展平为向量
    
    Args:
        obs_dict: 观测字典，包含：
            - block_translation (2D)
            - block_orientation (1D)
            - effector_translation (2D)
            - effector_target_translation (2D)
            - target_translation (2D)
            - target_orientation (1D)
    
    Returns:
        展平后的观测向量，形状为 (10,)
    """
    # 按照 IBC 的顺序展平（匹配 block_pushing.py 中的观测顺序）
    obs_list = [
        obs_dict.get('block_translation', [0.0, 0.0]),  # 2D
        obs_dict.get('block_orientation', [0.0]),  # 1D
        obs_dict.get('effector_translation', [0.0, 0.0]),  # 2D
        obs_dict.get('effector_target_translation', [0.0, 0.0]),  # 2D
        obs_dict.get('target_translation', [0.0, 0.0]),  # 2D
        obs_dict.get('target_orientation', [0.0]),  # 1D
    ]
    
    # 展平为 1D 数组
    obs_flat = np.concatenate([np.array(x).flatten() for x in obs_list], axis=0)
    return obs_flat.astype(np.float32)


class PushingStatesDataset(Dataset):
    """
    Block Pushing (states) 环境数据集
    从 JSON 文件加载轨迹数据
    """
    ...
    def __init__(
        ...
        for json_file in tqdm(self.json_files, desc="加载数据"):
            ...
            observations = data['trajectory']['observations']
            actions = np.array(data['trajectory']['actions'], dtype=np.float32)
            
            # 展平观测
            obs_flat_list = [flatten_observation(obs) for obs in observations]
            
            # ===== 先添加首帧重复的样本：[x1, x1] -> a1 =====
            if len(obs_flat_list) >= self.obs_seq_len:
                # 构建首帧观测（原始空间）
                first_obs = obs_flat_list[0]
            ...
            # ===== 继续原来的滑动窗口样本构造 =====
            for i in range(len(obs_flat_list) - 1):
                if i + self.obs_seq_len <= len(obs_flat_list):
                    # 构建观测序列
                    obs_seq = []
                    for j in range(self.obs_seq_len):
                        idx = i + j
                        if idx < len(obs_flat_list):
                            obs_seq.append(obs_flat_list[idx])
        ...
        if len(self.episodes) == 0:
            raise ValueError(
                ...
                f"确保 traj/ 目录下存在 traj_*.json 文件。\n"
                f"可以使用 pushing_states_data_generate.py 生成数据。"
            )
        ...
        print(f"观测均值范围: [{self.obs_mean.min():.3f}, {self.obs_mean.max():.3f}]")
        print(f"观测标准差范围: [{self.obs_std.min():.3f}, {self.obs_std.max():.3f}]")
...
def train_pushing_states_ebm(
    data_dir: str,
    output_dir: str,
    obs_seq_len: int = 2,
    hidden_dim: int = 128,  # 匹配 gin: MLPEBM.width = 128
    num_residual_blocks: int = 8,  # 匹配 gin: MLPEBM.depth = 16 (depth/2 = 8)
    dropout: float = 0.0,  # 匹配 gin: MLPEBM.rate = 0.0
    norm_type: str = None,  # 匹配 gin: ResNetLayer.normalizer = None
    batch_size: int = 512,  # 匹配 gin: train_eval.batch_size = 512
    ...
    grad_margin: float = 1.0,  # 匹配 gin: grad_penalty.grad_margin = 1.0
    grad_norm_type: str = 'inf',  # 匹配 IBC 默认
    square_grad_penalty: bool = True,
    grad_loss_weight: float = 1.0,
    ...
    """
    训练 Block Pushing (states) EBM 模型
    
    参数与 IBC 的 mlp_ebm_langevin.gin 配置对齐
    """
    print("=" * 60)
    print("训练 Block Pushing (states) EBM 模型")
    print("=" * 60)
    ...
    # 加载数据集
    print("加载数据集...")
    dataset = PushingStatesDataset(data_dir, obs_seq_len=obs_seq_len)
    ...
    # 计算观测维度（展平后的观测序列）
    obs_dim = dataset.episodes[0]['obs_seq'].shape[-1]  # 单个观测的维度（10）
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='训练 Block Pushing (states) EBM 模型')
    parser.add_argument('--data_dir', type=str, 
                       default=str(IBC_ROOT / 'data' / 'pushing_states'),
                       help='数据目录')
    parser.add_argument('--output_dir', type=str,
                       default=str(IBC_ROOT / 'models' / 'pushing_states'),
                       help='输出目录')
    ...
    train_pushing_states_ebm(
        data_dir=args.data_dir,
        output_dir=args.output_dir,
        obs_seq_len=2,  # 匹配 gin: train_eval.sequence_length = 2
        hidden_dim=128,  # 匹配 gin: MLPEBM.width = 128
        num_residual_blocks=8,  # 匹配 gin: MLPEBM.depth = 16 (depth/2 = 8)
        dropout=0.0,  # 匹配 gin: MLPEBM.rate = 0.0
        norm_type=None,  # 匹配 gin: ResNetLayer.normalizer = None
        ...
        grad_margin=1.0,  # 匹配 gin: grad_penalty.grad_margin = 1.0
        grad_norm_type='inf',  # 匹配 IBC 默认
